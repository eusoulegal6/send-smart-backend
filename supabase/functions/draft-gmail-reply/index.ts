...
// CORS: Allow all origins for development. Tighten by replacing '*' with your extension origin.
const corsHeaders = {
...
serve(async (req) => {
  ...
  try {
    ...
    const requiredString = ["subject", "senderName", "senderEmail", "latestMessage"];
    ...
    const optionalStrings = [
      "sourceUrl", "identity", "replyStyle", "knowledge", "signature", "extraInstructions",
    ];
    ...
    // threadMessages must be an array of strings if provided
    ...
    const threadContext =
      threadMessages.length > 0
        ? `\n\nPrevious messages in the thread (oldest first):\n${threadMessages.map((m: string, i: number) => `--- Message ${i + 1} ---\n${m}`).join("\n\n")}`
        : "";
...
Draft a reply now.`;
    ...
    console.log(`Calling Anthropic model=${model} subject="${subject}" sender="${senderEmail}"`);
    ...
    console.log(`Success: generated ${draft.length} chars for "${subject}"`);
